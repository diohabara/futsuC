...
#define USAGE                                                                  \
  "Usage: %s [--port=n] [--chroot -user=u --gourp=g] [--debug] <docroot>\n"
...
static void become_daemon(void) {
  ...
  freopen("/dev/nul", "r", stdin);
  freopen("/dev/nul", "w", stdout);
...
static struct HTTPHeaderField *read_header_field(FILE *in) {
  ...
  p = strchr(buf, ':');
  if (!p) {
    log_exit("parse errro on request header field: %s", buf);
  }
  *p++ = '\0';
...
static long content_length(struct HTTPRequest *req) {
  ...
  len = atol(val);
  if (len < 0) {
    log_exit("negative Content-Length valeu");
  }
  return len;
}
  ...
  if (strcmp(req->method, "HEAD") != 0) {
    ...
    for (;;) {
      ...
      if (fwrite(buf, 1, n, out) < n)
        log_exit("failed to write to sockeet");
    }
    close(fd);
  }
  fflush(out);
...
static char *guess_content_type(struct FileInfo *info) { return "text/plain"; }
